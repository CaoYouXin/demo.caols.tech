<template>
  <div class="chart" v-d3chart=""></div>
</template>

<script>
  import * as d3 from 'd3';
  import Vue from 'vue';
  import _ from '../directives/D3Chart';

  export default {
    name: 'user-chart-reverse',
    data: () => ({
      htmlValue: null,
      path: '',
    }),
    methods: {
      createChart(el) {
        ...
        d3.json('static/data/user-chart-data.json', function fn(error, rawData) {
          if (error) {
            console.error(error);
            return;
          }

          const data = [].map.call(rawData, function fn(d) {
            return {
          ...
          d3.json('static/data/user-chart-markers.json', function fn(error, markerData) {
            if (error) {
              console.error(error);
              return;
            }

            const markers = [].map.call(markerData, function fn(marker) {
              return {
      ...
      startTransition(el) {
        ...
        const chartWidth = svg.attr('data-rel-chart-width');
        const chartHeight = svg.attr('data-rel-chart-height');
        ...
        svg.select('#rect-clip > rect').transition()
      ...
      makeChart(data, markers) {
        ...
        const x = d3.scaleTime().range([0, chartWidth])
            .domain(d3.extent(data, function fn(d) {
              return d['date'];
            })),
          y = d3.scaleLinear().range([chartHeight, 0])
            .domain([0, d3.max(data, function fn(d) {
              return d['pct95'];
            })]);
        ...
        const svg = d3.select(node).append('svg')
          .attr('data-rel-chart-width', chartWidth)
          .attr('data-rel-chart-height', chartHeight)
          .attr('width', svgWidth)
